...
``ev.soc_plugin``, ``{"name": "<name>", ...kwargs}`` — see ``ev.py``)
anywhere the loader allows it; at run time the registered function is
called with a ``PluginContext`` and the remaining keys as kwargs.

Three EV helpers are registered here, ported from
``test_ev_harness_v6.py``: ``remainder_soc`` (backs into an
``actual_soc``/``wished_level`` pair that leaves a sub-minimum-duty
remainder on top of N full-power intervals), ``ready_u_zero`` (pins the
ready deadline just before the end of interval ``u=0``) and
``day_rollover`` (drives day_ahead.py's short ``"HH:MM:SS"`` ready-time
parsing across midnight).
"""
...
def _predict_ready_rollover(start_dt: dt.datetime, hh_mm_ss: str) -> dt.datetime:
    ...
    (the ``len(ready_str) <= 9`` branch): same-day ``HH:MM`` unless it's
    at/before ``start_dt``'s own ``HH:MM``, in which case it rolls to
    tomorrow. Ported verbatim from ``test_ev_harness_v6._predict_ready_rollover``."""
...
def _ready_u_zero(ctx: PluginContext, buffer_s: int = 45) -> dict:
    ...
    ends of the window had to be pinned from one real ``now()`` snapshot so
    the achievable charging window couldn't shrink depending on real
    wall-clock timing. Here the clock is frozen at the scenario's own
    ``start`` — already a grid boundary (``vocabulary.parse_start``
    enforces it) — so ``start`` already *is* the beginning of interval
    ``u=0`` and no snapshotting is needed: just place ``ready`` just before
    that interval's end."""
...
def _day_rollover(ctx: PluginContext, hour: int = 1, minute: int = 0) -> dict:
    ...
    JSON supplies its own late-clock ``start`` (e.g. 23:15); this computes
    the short ``"HH:MM:SS"`` override string and the predicted resolved
    datetime, so the case-level echo check has something to compare
    against."""
...
def _remainder_soc(
    ...
    ``full_intervals`` whole intervals at the EV's own top-stage power plus
    a tiny ``remainder_kwh`` — engineered to force a sub-minimum-duty
    sliver rather than landing on it by luck. Reads the EV's real
    ``capacity``/``charge_stages``/``charge_three_phase`` from ``ctx.config``
    so the corpus stays correct if that config ever changes."""
